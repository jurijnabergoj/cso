...
class SAM3DCountPredictor(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            slat_features_container: (B, N_points, 8)
            slat_features_object: (B, N_points, 8)
        ...
        # Pool sparse latent features
        slat_container_pooled = torch.cat([
            slat_features_container.max(dim=1)[0],
            slat_features_container.mean(dim=1)
        ], dim=-1)
        slat_object_pooled = torch.cat([
            slat_features_object.max(dim=1)[0],
            slat_features_object.mean(dim=1)
        ], dim=-1)
        combined_slat = torch.cat([slat_container_pooled, slat_object_pooled], dim=-1)
